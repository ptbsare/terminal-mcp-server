...
export class CommandExecutor {
    ...
    async isConnectionActive(client) {
    ...
    async disconnectSession(sessionKey) {
        const session = this.sessions.get(sessionKey);
        if (session) {
            if (session.client) {
    ...
    async disconnect() {
        const disconnectPromises = Array.from(this.sessions.keys()).map(sessionKey => this.disconnectSession(sessionKey));
        await Promise.all(disconnectPromises);
        this.sessions.clear();
    }
    async parseSshConfig(alias) {
        ...
        try {
            ...
            if (hostConfig) {
                const parsed = {};
                hostConfig.config.forEach((item) => {
